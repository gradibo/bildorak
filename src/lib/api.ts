...
// ── 릴리스 관리(1차 슬라이스) - model.rs::ReleaseRecord 와 1:1 대응. commands.rs 의 릴리스 관련
// 커맨드만 부른다. ──

/** 이 프로젝트의 릴리스 기록 전체(최신순, createdAt desc - 서버가 정렬해서 돌려준다). */
export function listReleases(projectId: string): Promise<ReleaseRecord[]> {
  return invoke<ReleaseRecord[]>("list_releases", { projectId });
}
...
export function createRelease(
  ...
  return invoke<ReleaseRecord>("create_release", { projectId, version, buildNumber, channel, status, notes });
}

/** 릴리스 기록을 수정한다 - status 가 "released"로 처음 바뀌면 releasedAt 은 서버가 알아서 채운다
 * (프론트는 넘기지 않는다). */
export function updateRelease(
  ...
  return invoke<ReleaseRecord>("update_release", { releaseId, version, buildNumber, channel, status, notes });
}

/** 릴리스 기록을 삭제한다 - 이 기록만 지워질 뿐 실제 스토어 상태나 프로젝트 파일은 건드리지 않는다. */
export function deleteRelease(releaseId: string): Promise<void> {
  return invoke<void>("delete_release", { releaseId });
}

/** 프로젝트 폴더의 pubspec.yaml 을 지금 다시 읽어 버전/빌드번호를 돌려준다(등록 시점 스냅샷이 아니라
 * 실시간) - 새 릴리스 폼의 버전 pre-fill 에 쓴다. 실패하면 프론트가 project.version 스냅샷으로 조용히
 * 폴백한다. */
export function getProjectCurrentVersion(projectId: string): Promise<ProjectCurrentVersion> {
  return invoke<ProjectCurrentVersion>("get_project_current_version", { projectId });
}
