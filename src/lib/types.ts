...
// ── 릴리스 관리(1차 슬라이스) - model.rs 의 ReleaseChannel/ReleaseStatus/ReleaseRecord/
// ProjectCurrentVersion 과 1:1 대응. ──

/** 릴리스가 올라가는 채널 - 닫힌 집합(model.rs::ReleaseChannel 과 같은 snake_case 값). */
export type ReleaseChannel = "app_store" | "play_store" | "github" | "other";

/** 릴리스 진행 상태 - 닫힌 집합. "released"로 처음 바뀌면 서버가 releasedAt 을 자동으로 채운다
 * (commands.rs::update_release). */
export type ReleaseStatus = "preparing" | "submitted" | "approved" | "rejected" | "released";

/** 등록된 릴리스 기록 한 건 - list_releases/create_release/update_release 커맨드가 그대로 반환한다. */
export interface ReleaseRecord {
  ...
  /** 자유 텍스트(단순 textarea) - 심사 코멘트, 체크리스트 등. */
  notes: string;
  createdAt: string;
  updatedAt: string;
  /** status 가 "released"로 처음 바뀐 시점(RFC3339) - 그 전까지는 필드 자체가 없다(model.rs 의
   * skip_serializing_if, SigningKeyRecord::vaultPath 와 같은 패턴). */
  releasedAt?: string;
}
...
/** get_project_current_version 커맨드 반환 형태 - 새 릴리스 폼의 버전 pre-fill 에 쓴다. */
export interface ProjectCurrentVersion {
  version: string | null;
  buildNumber: string | null;
}
