...
/// 파일보다 더 맞는 자리가 없어서다 — 명령 5개 전부(apps/build/status/keys/doctor)를 아우르는 목록이라
...
mod tests {
    ...
    #[test]
    fn cli_manifest_covers_all_five_scope_a_commands_in_order() {
        let manifest = cli_manifest();
        let names: Vec<&str> = manifest.iter().map(|d| d.name.as_str()).collect();
        assert_eq!(names, vec!["apps", "build", "status", "keys", "doctor"]);
        for doc in &manifest {
